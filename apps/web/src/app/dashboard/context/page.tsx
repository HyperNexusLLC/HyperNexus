...
export default function ContextDashboard() {
	...
	const promptText =
		typeof promptQuery.data === "string" ? promptQuery.data : "";
			...
			<div className="flex items-start justify-between">
				<div>
					...
					<p className="text-zinc-500 mt-2">
						Manage the set of files that are injected into the HyperNexus
						context prompt for active AI sessions.
					</p>
				</div>
				<Button
					variant="outline"
					...
					<div className="flex items-center justify-between">
						...
						{contextFiles.length > 0 && (
							<Button
								...
								onClick={() => {
									if (confirm("Remove all context files?"))
									...
									{contextFiles.map((file) => (
										...
										>
											...
											<Button
												...
												onClick={() =>
													removeMutation.mutate({ filePath: file })
												}
												disabled={
													removeMutation.isPending &&
													(
														removeMutation.variables as
															| { filePath?: string }
															| undefined
													)?.filePath === file
												}
				...
				<div className="space-y-4">
					...
					<Card className="bg-zinc-950 border-zinc-800">
						<CardHeader className="pb-2 border-b border-zinc-800">
							<CardTitle className="text-xs text-zinc-500 font-mono">
								context_prompt.txt
							</CardTitle>
						</CardHeader>
						<CardContent className="p-0">
