...
function getBrowserExtensionArtifactSummary(
	artifacts?: SystemInstallSurfaceArtifactInput[] | null,
): {
	readyCount: number;
	totalCount: number;
	...
	const normalizedArtifacts = (
		...
		(artifact): artifact is SystemInstallSurfaceArtifactInput =>
			Boolean(artifact) &&
			typeof artifact === "object" &&
			typeof artifact.id === "string",
	);
	const relevantArtifacts = normalizedArtifacts.filter((artifact) =>
		BROWSER_EXTENSION_SURFACE_IDS.includes(
			artifact.id as (typeof BROWSER_EXTENSION_SURFACE_IDS)[number],
		),
	);
...
function getBrowserExtensionArtifactDetail(
	artifacts?: SystemInstallSurfaceArtifactInput[] | null,
): string {
...
function isStartupTelemetryConnecting(
...
export function buildSystemEnvironmentRows(
	startupStatus: SystemStartupStatusInput | undefined,
): SystemEnvironmentRow[] {
	const nodeEnv =
		typeof startupStatus?.runtime?.nodeEnv === "string"
			? startupStatus.runtime.nodeEnv.trim()
			: "";
	const platform =
		typeof startupStatus?.runtime?.platform === "string"
			? startupStatus.runtime.platform.trim()
			: "";
	const version =
		typeof startupStatus?.runtime?.version === "string"
...
export function buildSystemStartupNotice(
	startupStatus: SystemStartupStatusInput | undefined,
): SystemStartupNotice | null {
	if (isStartupTelemetryConnecting(startupStatus)) {
		return {
			title: "Connecting to live telemetry",
			detail:
				"Waiting for the first live startup snapshot from core so this page can replace neutral placeholders with the authoritative readiness contract.",
			tone: "info",
		};
	}

	const summary =
		typeof startupStatus?.summary === "string"
	...
	if (startupStatus?.status === "degraded") {
		return {
			...
			detail:
				summary ||
				"Live startup telemetry is unavailable, so HyperNexus is showing config-backed compatibility state instead of the full core startup contract.",
			tone: "warning",
		};
	}
...
function getResidentMcpDetail(startupStatus: SystemStartupStatusInput): string {
	...
	const residentReady =
		...
		((aggregator?.liveReady ?? aggregator?.ready) &&
			residentConnectedCount >= residentServerCount);
...
function getExtensionBridgeDetail(
	startupStatus: SystemStartupStatusInput,
): string {
	const extensionBridge = startupStatus.checks?.extensionBridge;
	const clientCount = extensionBridge?.clientCount ?? 0;
	const acceptingConnections =
		extensionBridge?.acceptingConnections ?? extensionBridge?.ready;
	const hasConnectedClients =
		extensionBridge?.hasConnectedClients ?? clientCount > 0;
...
function getExecutionEnvironmentDetail(
	startupStatus: SystemStartupStatusInput,
): string {
...
function getMemoryContextDetail(
	...
	if (memory?.ready) {
		if (claudeMem?.enabled) {
			return "Memory manager initialized and HyperNexus default sections are ready";
		}
	...
	if (claudeMem?.enabled) {
		if (!claudeMem.storeExists) {
			return "Memory manager is initialized, but HyperNexus store has not been created yet";
		}
		...
		return "Memory manager is initialized, but HyperNexus readiness is still pending";
	}

	return "Memory manager is present, but agent context wiring is still finishing";
}

function getResidentRuntimeStatus(
	startupStatus: SystemStartupStatusInput,
): "Operational" | "Pending" {
	const aggregator = startupStatus.checks?.mcpAggregator;
	...
	return (aggregator?.residentReady ??
		(liveReady && residentConnectedCount >= residentCount))
		? "Operational"
		: "Pending";
}
...
export function buildSystemStatusCards(
	...
	const startupChecks = startupStatus
		? buildSystemStartupChecks(startupStatus, installSurfaceArtifacts)
		: [];
	const startupTelemetryConnecting =
		isStartupTelemetryConnecting(startupStatus);
	const startupSummary =
		typeof startupStatus?.summary === "string"
			? startupStatus.summary.trim()
			: "";
	const extensionArtifactSummary = getBrowserExtensionArtifactSummary(
		installSurfaceArtifacts,
	);
	const bridge = startupStatus?.checks?.extensionBridge;
	const cachedServerCount =
		startupStatus?.checks?.mcpAggregator?.advertisedServerCount ??
		startupStatus?.checks?.mcpAggregator?.persistedServerCount ??
		0;
	const cachedToolCount =
		startupStatus?.checks?.mcpAggregator?.advertisedToolCount ??
			...
			status: startupTelemetryConnecting
				...
				: startupStatus?.status === "degraded"
					...
					: startupStatus?.checks
						? startupChecks.every((check) => check.status === "Operational")
							? "Ready"
							: "Warming"
						: "Loading",
			detail: startupTelemetryConnecting
...
export function buildSystemComponentHealthRows(
	...
	const inventoryReady =
		startupStatus?.checks?.mcpAggregator?.inventoryReady ?? false;
	const advertisedServerCount =
		startupStatus?.checks?.mcpAggregator?.advertisedServerCount ??
		startupStatus?.checks?.mcpAggregator?.persistedServerCount ??
		0;
	const persistedServerCount =
	...
	return [
		{
			...
			status: startupTelemetryConnecting
				...
				: startupStatus?.status === "running"
					? "Operational"
					: "Pending",
			latency: startupTelemetryConnecting
				? "connecting"
				: startupStatus?.ready
					? "ready"
					: "warming",
		...
		},
		{
			...
			latency: startupTelemetryConnecting
				? "connecting"
				: `${advertisedServerCount}/${persistedServerCount} cached`,
			detail: startupTelemetryConnecting
				? "Waiting for the first live startup snapshot from core."
				: inventoryReady
		...
		},
		{
			...
			latency: browserStatus?.available
				? `${browserStatus?.pageCount ?? 0} pages`
				: "-",
		...
		},
		{
			...
			status:
				(bridge?.acceptingConnections ?? bridge?.ready)
					? "Operational"
					: "Pending",
...
export function buildSystemStartupChecks(
	...
	const memory = startupStatus.checks?.memory;
	const extensionBridge = startupStatus.checks?.extensionBridge;
	const extensionBridgeOperational =
		extensionBridge?.acceptingConnections ?? extensionBridge?.ready;
	const persistedToolCount =
		aggregator?.advertisedToolCount ?? aggregator?.persistedToolCount ?? 0;
	const sessionCount =
		startupStatus.checks?.sessionSupervisor?.sessionCount ?? 0;
	...
	return [
		...
		{
			name: "Session Restore",
			status: startupStatus.checks?.sessionSupervisor?.ready
				? "Operational"
				: "Pending",
