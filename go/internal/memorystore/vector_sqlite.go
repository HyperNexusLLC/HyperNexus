...
func (s *VectorStore) Commit(ctx context.Context, entry controlplane.L2VaultRecord) error {
	...
	if len(entry.Embedding) > 0 {
		...
		_, err = s.db.ExecContext(ctx, `
			...
			VALUES (?, ?)
			ON CONFLICT(id) DO UPDATE SET embedding = excluded.embedding
		`, entry.ID, encodeVec(vec))
		if err != nil {
			return fmt.Errorf("memorystore commit auto-embed: %w", err)
		}
	}
	return nil
}
...
func (s *VectorStore) detectConflicts(ctx context.Context, entry controlplane.L2VaultRecord) ([]controlplane.L2VaultRecord, error) {
	...
	var conflicts []controlplane.L2VaultRecord
	for _, existing := range results {
		if existing.ID == entry.ID || existing.Kind == "superseded" {
			continue
		}
		if isContradiction(entry.Content, existing.Content) {
...
func sharedKeywords(a, b string) int {
	...
	for _, w := range strings.Fields(a) {
		if len(w) > 3 { wordsA[w] = true }
	}
	count := 0
	for _, w := range strings.Fields(b) {
		if len(w) > 3 && wordsA[w] { count++ }
	}
	return count
}
...
func slicesOverlap(a, b []string) bool {
	...
	for _, v := range b { if set[v] { return true } }
	return false
}
...
// Uses SimHash: each word hashes to a dimension, accumulating +/- contributions.
// Not semantically meaningful, but enables cosine similarity for dedup/conflict detection.
func simpleEmbed(text string, dim int) []float32 {
	...
	for _, word := range words {
		...
		if idx < 0 { idx = -idx }
		// Alternate +/- based on second hash bit
		if h&2 == 0 {
			vec[idx] += 1.0
		} else {
			vec[idx] -= 1.0
	...
	// L2 normalize
	var norm float64
	for _, v := range vec { norm += float64(v) * float64(v) }
	if norm > 0 {
		norm = math.Sqrt(norm)
