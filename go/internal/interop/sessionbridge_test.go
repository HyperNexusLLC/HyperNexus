...
func TestResolveTRPCBasesNoUpstreamWhenNoEnv(t *testing.T) {
	t.Setenv("HYPERNEXUS_TRPC_UPSTREAM", "")
	bases := ResolveTRPCBases("")
	if len(bases) != 0 {
		t.Fatalf("expected no upstream bases when env not set (Go kernel is sole control plane), got %v", bases)
	}
}
