...
func (s *Server) handleSessionContext(w http.ResponseWriter, r *http.Request) {
	...
	// Prefer bootstrap response values over URL params for the tool query
	effectiveGoal := activeGoal
	effectiveObjective := lastObjective
	if strings.TrimSpace(bootstrap.Goal) != "" {
		effectiveGoal = strings.TrimSpace(bootstrap.Goal)
	}
	if strings.TrimSpace(bootstrap.Objective) != "" {
		effectiveObjective = strings.TrimSpace(bootstrap.Objective)
	}
	query := strings.TrimSpace(strings.Join([]string{effectiveObjective, effectiveGoal}, " "))
