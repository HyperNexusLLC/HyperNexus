...
- **GitHub remote**: The repo URL has moved to `https://gitlab.com/HyperNexusLLC/HyperNexus.git` (GitHub redirects from old `NexusSoftMDMA/HyperNexus` URL)
...
- **Local Module Replacing**: Setting `replace gitlab.com/HyperNexusLLC/HyperNexus => ../` and requiring `gitlab.com/HyperNexusLLC/HyperNexus v0.0.0` in `go/go.mod` allows the Go sidecar module to import `"gitlab.com/HyperNexusLLC/HyperNexus/tools"` without invoking network proxy lookups.
