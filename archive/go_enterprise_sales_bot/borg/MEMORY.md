...
- **GitHub remote**: The repo URL has moved to `https://gitlab.com/hypernexus-org/HyperNexus.git` (GitHub redirects from old `NexusSoftMDMA/HyperNexus` URL)
...
- **Local Module Replacing**: Setting `replace gitlab.com/hypernexus-org/HyperNexus => ../` and requiring `gitlab.com/hypernexus-org/HyperNexus v0.0.0` in `go/go.mod` allows the Go sidecar module to import `"gitlab.com/hypernexus-org/HyperNexus/tools"` without invoking network proxy lookups.
