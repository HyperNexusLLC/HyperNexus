...
## Goal
Parse `~/.hypernexus/mcp-cache.json` (the full 26MB catalog with ~7000 entries) and
`~/.hypernexus/mcp.json` (60 active high-priority servers) to build a ranked list of
...
cache_path = Path.home() / '.hypernexus' / 'mcp-cache.json'
active_path = Path.home() / '.hypernexus' / 'mcp.json'
```
