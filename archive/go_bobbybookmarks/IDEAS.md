...
- **Pivot:** Instead of centralizing the research worker on a single instance, create a distributed scraping task queue that can be operated by lightweight client worker nodes. Nodes pull target URLs from an external Redis/Supabase instance and return results into `hypernexus.db`.

## 2. Graph Database Transition
- **Refactor:** Migrate the complex relational logic of `hypernexus.db` and the lost `catalog.db` relationships into Neo4j or another graph-based database to more easily construct the Knowledge Graph backbone highlighted in `VISION.md`.
